...
def parallel_plot(x1: np.ndarray, y1: np.ndarray, x2: np.ndarray, y2: np.ndarray,
    ...
    if x1.shape != y1.shape or min(x1.shape) <= 1 or x2.shape != y2.shape or min(x2.shape) == 0 or len(np.unique(x1)) != len(x1) or len(np.unique(x2)) != len(x2):
        return None
...
def log_plot(x: np.ndarray, y: np.ndarray, xlabel: str, ylabel: str, title: str, log_axis: str):
    ...
    if x.shape == y.shape and min(x.shape) != 0 and len(np.unique(x)) == len(x) :
        plt.figure(1)
        plt.plot(x,y)
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
